...
class MainActivity : AppCompatActivity() {
    ...
    private fun loadNews() {
        ...
        val apiKey = "2fc4998785dd4cdf9f0298c3c617c4ae" // تأكد من استخدام API Key الصحيح

        if (selectedCountry.isNullOrEmpty()) {
            // جلب كل الأخبار عند تحديد "الكل"
            newsService.getNews(query = "all", apiKey = apiKey).enqueue(object : Callback<News> {
            ...
            // جلب الأخبار من الدولة المحددة
            newsService.getNewsByCountry(selectedCountry, apiKey).enqueue(object : Callback<News> {
                override fun onResponse(call: Call<News>, response: Response<News>) {
                    if (response.isSuccessful && response.body() != null) {
                        val news = response.body()
                        val articles = news?.articles ?: ArrayList()
